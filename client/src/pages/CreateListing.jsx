...
const CreateListing = () => {
    ...
    const handleImageSubmit = (e) => {
        if (files.length > 0 && (files.length + formData.imageUrls.length) < 7) {
            ...
            Promise.all(promises).then((urls) => {
                ...
                setImageUploadError('Image uoload failed! (2 mb max per image)');
                setUploading(false);
            })
        }
        else {
    ...
    return (
        <main className='p-3 max-w-4xl mx-auto'>
            <h1 className='text-3xl text-[#003C43] capitalize font-semibold text-center my-7 italic'>Create a Listing !</h1>
            <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
                <div className='flex flex-col gap-4 flex-1'>
                    <input type='text' placeholder='Name' className='border p-3 rounded-lg bg-[#FEFAF6]'
                        id='name' minLength='12' maxLength='62' onChange={handleChange} value={formData.name} required />
                    <textarea type='text' placeholder='Description' className='border p-3 rounded-lg bg-[#FEFAF6]'
                        id='description' onChange={handleChange} value={formData.description} required />
                    <input type='text' placeholder='Address' className='border p-3 rounded-lg bg-[#FEFAF6]'
                ...
                <div className='flex flex-col flex-1 gap-4'>
                    ...
                    <button disabled={loading || uploading} className='bg-[#135D66] text-white rounded-lg p-3 uppercase hover:opacity-95 disabled:opacity-80'>
